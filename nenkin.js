...
class SlimeMoldCycle {
  ...
  _animatePhase3_MyceliumGrowth() {
    ...
    if (grow >= 1 && this.phaseTime > 3.5) {
      ...
      if (!this.amoeba) {
        const sphereGeo = new THREE.SphereGeometry(1.5, 32, 32);
        const sphereMat = new THREE.MeshStandardMaterial({ color: this.color, roughness: 0.8, metalness: 0.1, transparent: true, opacity: 0.85, emissive: this.color, emissiveIntensity: 0.5 });
        this.amoeba = new THREE.Mesh(sphereGeo, sphereMat);
        this.scene.add(this.amoeba);
      }
  ...
  _animatePhase4_AmoebaGrowth() {
    let appear = Math.min(1, this.phaseTime / 2.0);
    this.amoeba.scale.setScalar(appear);
    this.myceliumLines.forEach(line => {
